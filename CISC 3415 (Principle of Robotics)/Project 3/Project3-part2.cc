...
int main(int argc, char *argv[]) {  
	...
	while(true) {    
		...
		// if not bumped, roomba sets its turnrate using the sum of sensor range.
		else {
			double reach = sp.MinLeft() + sp.MinRight(); 
			//roomba stops when it will be surrounded by the walls
			//End of program
			...
			//in the straight zones, roomba just go forward with +-0.2 turnrate
			else if (reach > 0 && reach < 2.5) {
			...
			//at the corners, roomba turn left or right.
			//if left reach is longer than right, turn left
			//if left raeach is shorter than right, turn right
			else {
				if (sp.MinLeft() < sp.MinRight()) {
					speed = 1;
					turnrate = -1;
